...
echo "🤖 RECATEGORIZACIÓN MEJORADA v3 - Hasta 3 categorías\n";
echo "=====================================\n";
echo "Offset: {$offset}\n";
echo "Límite: {$limit}\n";
...
function analyze_product_intelligent($title, $description) {
    ...
    // INMOBILIARIA (745) - Devolver array de IDs
    if (preg_match('/\b(inmueble|vivienda|apartamento|propiedad|terreno|parcela)\b/i', $text) ||
        ...
        // Determinar subcategoría de Alquiler
        ...
        // Determinar subcategoría de Venta
        if (preg_match('/\b(venta|vender|se vende|en venta|traspaso)\b/i', $text)) {
            if (preg_match('/\b(atico|aticos)\b/i', $text)) {
...
foreach ($products as $product) {
    ...
    // Analizar - ahora devuelve array de IDs directamente
    $categories_to_assign = analyze_product_intelligent($title, $description);
