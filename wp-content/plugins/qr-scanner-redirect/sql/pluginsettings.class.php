...
class QSR_PluginSettings {
     ...
     * @return bool
     */
    public function is($name='', $group)
    {
      $str = "SELECT COUNT(`id`) as count FROM `%s` WHERE `name` like '%s' AND `group` like '%s'";
      $sql = sprintf($str, $this->tablename, $name, $group);
    ...
    public function fetchPost(string $group)
    {
      ...
      foreach ($_values as $key => $value) {
        if(!$value->change && $this->is($key, $group)){
          switch ($value->type) {
            case 'boolean':
              $value->value = 0;
              break;
